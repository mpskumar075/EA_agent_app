...
export const categories = {
	...
	mutations: {
		...
		updateRecord(state, record) {
			const item = state.records.find(item => item['category'] === record['category']);
			if(item){
		...
		deleteRecord(state, id) {
			if(Array.isArray(id)){
				id.forEach((itemId) => {
					let itemIndex = state.records.findIndex(item => item['category'] == itemId);
					if(itemIndex!=-1){
				...
				let itemIndex = state.records.findIndex(item => item['category'] == id);
				if(itemIndex!=-1){
					state.records.splice(itemIndex, 1);
				}
			}
