                        ...
                        <div class="col-12 col-md-auto ">
                            <div class="">
                                <div class="row  items-center q-col-gutter-sm q-px-sm">
                                    <div class="col">
                                        <div class="text-h6 text-primary">{{ $t('revenue') }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-auto col-12 ">
                            <q-btn       :rounded="false"  size=""  color="primary" no-caps  unelevated   :to="`/revenue/add`" class="full-width">
                            ...
                            <div>
                                <template v-if="showBreadcrumbs && $route.query.tag">
                                    <q-breadcrumbs class="q-pa-md">
                                        <q-breadcrumbs-el icon="arrow_back" class="text-capitalize" :label="$route.query.tag" to="/revenue"></q-breadcrumbs-el>
                                    ...
                                    <div class="row q-col-gutter-x-md">
                                        ...
                                        <div class="col">
                                            ...
                                            <template >
                                                ...
                                                :bordered="false"
                                                :columns="$menus.RevenueTableHeaderItems" 
                                                :data="records"
                                                binary-state-sort
                                                ...
                                                <template v-slot:body="props">
                                                    <q-tr :class="{selected: isCurrentRecord(props.row)}" :props="props">
                                                        ...
                                                        <q-td  key="revenue_id" :props="props">
                                                            <q-btn padding="xs"   :rounded="false"  color="primary"  no-caps  unelevated   flat :to="`/revenue/view/${props.row.revenue_id}`">{{ props.row.revenue_id }}</q-btn>
                                                        </q-td>
                                                        <q-td  key="amount" :props="props">
                                                            {{ props.row.amount }}
                                                        </q-td>
                                                        <q-td  key="agent_id" :props="props">
                                                            <q-btn v-if="props.row.agent_id" @click="openPageDialog({ page: 'users/view', url: `/users/view/${props.row.agent_id}` }, { closeBtn: true })" padding="xs" color="blue-1" unelevated text-color="blue" no-caps >
                                                                <q-icon name="visibility" size="xs"  class="q-mr-xs"></q-icon>  {{ $t('users') }}
                                                            </q-btn>
                                                        </q-td>
                                                        <q-td  key="patient_id" :props="props">
                                                            <q-btn v-if="props.row.patient_id" @click="openPageDialog({ page: 'patient/view', url: `/patient/view/${props.row.patient_id}` }, { closeBtn: true })" padding="xs" color="blue-1" unelevated text-color="blue" no-caps >
                                                                <q-icon name="visibility" size="xs"  class="q-mr-xs"></q-icon>  {{ $t('patient') }}
                                                            </q-btn>
                                                        </q-td>
                                                        <q-td  key="date_paid" :props="props">
                                                            {{ props.row.date_paid }}
                                                        </q-td>
                                                                ...
                                                                <q-btn icon="menu" padding="xs" round flat color="grey">
                                                                    <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                                        <q-list dense rounded nav>
                                                                            <q-item link clickable v-ripple :to="`/revenue/view/${props.row.revenue_id}`">
                                                                                <q-item-section>
                                                                                    <q-icon color="primary"  size="sm" name="visibility"></q-icon>
                                                                                </q-item-section>
                                                                                <q-item-section>{{ $t('view') }}</q-item-section>
                                                                            </q-item>
                                                                            <q-item link clickable v-ripple :to="`/revenue/edit/${props.row.revenue_id}`">
                                                                                <q-item-section>
                                                                                    <q-icon color="positive"  size="sm" name="edit"></q-icon>
                                                                                </q-item-section>
                                                                                <q-item-section>{{ $t('edit') }}</q-item-section>
                                                                            </q-item>
                                                                            <q-item link clickable v-ripple @click="deleteItem(props.row.revenue_id)">
                                                                                <q-item-section>
...
<script>
	...
	export default {
		...
		computed: {
			...
			records: {
				get: function () {
					return this.$store.getters["revenue/records"];
				},
				set: function (value) {
					this.$store.commit("revenue/setRecords", value);
				},
			},
			currentRecord: {
				get: function () {
		...
		methods: {
			...mapActions("revenue", ["fetchRecords", "deleteRecord"]),
			load: function() {
				if (!this.loading) {
					this.loading = true;
